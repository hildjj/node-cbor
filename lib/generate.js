...
/**
 * Generate CBOR from a JavaScript object.
 *
 * @class Generator
 * @constructor
 * @param {object} options
 *        genTypes: an array of Type, generatorFunction...
 */
function Generator(options) {
  ...
  /**
   * Pack an object into the given BufferStream.  This is unsafe because
   * it doesn't check its inputs; that is done in pack(), which is what
   * most users should call.  This is provided for those that want to
   * write new semantic tag packing functions.
   *
   * @param  {any} obj  The item to pack
   * @param  {BufferStream} bufs The BufferStream that will receive the packed bytes
   */
  this.unsafePack = function(obj, bufs) {
    switch(typeof(obj)) {
      case 'number':   _packNumber(obj, bufs); break;
      case 'string':   _packString(obj, bufs); break;
      case 'boolean':   bufs.writeUInt8(obj ? 0xd9 : 0xd8); break;
      case 'undefined': bufs.writeUInt8(0xdb); break;
      case 'object':   _packObject(this, obj, bufs); break;
...
/**
 * Pack a given object into a Buffer or BufferStream as a CBOR data item.
 *
 * @param  {any} obj  The object to pack.
 * @param  {BufferStream} bufs Optional BufferStream to pack into.
 * @return {Buffer} Optional.  If bufs is specified, returns undefined, otherwise
 *     a Buffer sized to fit the packed representation.
 */
...
/**
 * Convenience function.  If you are going to be packing large numbers of
 * objects, create a Generator object and pack from it.
 *
 * @param  {any} obj  The object ot pack
 * @param  {BufferStream} bufs Optional.  BufferStream to pack into.
 * @return {[type]}      [description]
 */
exports.pack = function pack(obj, options, bufs) {
  if (!bufs) {
    if (options instanceof BufferStream) {
      bufs = options;
      options = {};
    }
  }
