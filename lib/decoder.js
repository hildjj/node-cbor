...
const STRING_TABLE = Symbol('String table')
const ERROR = Symbol('error')
const NOT_FOUND = Symbol('not found')

function parentArray(parent, typ, count, stringTable) {
  const a = []
  a[COUNT] = count
  a[SYMS.PARENT] = parent
  a[MAJOR] = typ
  a[STRING_TABLE] = stringTable;
  return a
}

function parentBufferStream(parent, typ, stringTable) {
  const b = new NoFilter
  b[COUNT] = -1
  b[SYMS.PARENT] = parent
  b[MAJOR] = typ
  b[STRING_TABLE] = stringTable;
  return b
}
  ...
  *_parse() {
    ...
    while (true) {
      ...
      switch (mt) {
        ...
        case MT.UTF8_STRING:
          switch (val) {
            case 0:
              ...
              val = (mt === MT.BYTE_STRING) ? new Buffer.alloc(0) : ''
              break
            case -1:
              this.emit('start', mt, SYMS.STREAM, parent_major, parent_length)
              parent = parentBufferStream(parent, mt, stringTable)
              depth++
              continue
            default:
              this.emit('start-string', mt, val, parent_major, parent_length)
          ...
          if (stringTable !== null ) { 
            var minLength =  utils.minCborLength(stringTable.length)
            if (val.length >= minLength && parent && parent[COUNT] !== -1) {
        ...
        case MT.MAP:
          switch (val) {
            ...
            case -1:
              this.emit('start', mt, SYMS.STREAM, parent_major, parent_length)
              parent = parentArray(parent, mt, -1, stringTable)
              depth++
              continue
            default:
              this.emit('start', mt, val, parent_major, parent_length)
              parent = parentArray(parent, mt, val * (mt - 3), stringTable)
              depth++
              continue
          }
          break
        case MT.TAG:
          ...
          parent.push(val)
          depth++
          if (val === constants.TAG.STRINGREF_NAMESPACE) {
            stringTable = []  // Reset string table to empty array
          }
          continue
      ...
      while (parent != null) {
        ...
        if (Array.isArray(parent)) {
          switch (parent[MAJOR]) {
            ...
            case MT.MAP:
              ...
              break
            case MT.TAG:
              // Postprocessing of tagged values
              val = parent[1] // Get wrapped value
              if (parent[0] === constants.TAG.STRINGREF) {
                if (typeof val === "number" && val >= 0 && val < stringTable.length) {
                  val = stringTable[val];
                } else {
                  throw new Error("Invalid stringref value")
                }
              }
              else if (parent[0] === constants.TAG.STRINGREF_NAMESPACE) {
                val = parent[1]; // No conversion for STRINGREF_NAMESPACE
              } else {
        ...
        const old = parent
        stringTable = parent[STRING_TABLE]
        parent = parent[SYMS.PARENT]
        delete old[SYMS.PARENT]
        delete old[MAJOR]
        delete old[STRING_TABLE]
        delete old[COUNT]
      }
      if (!again) {
        return val
