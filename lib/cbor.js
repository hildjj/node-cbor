  ...
  function _unpack(bs, cb, tagged) {
    bs.wait(1, function(er, buf) {
      ...
      if (additionalBytes >= 0) {
        //process.stderr.write("additional: " + (1 << additionalBytes) + "\n")
        bs.wait(1 << additionalBytes, function(er, buf) {
          //process.stderr.write("len: " + buf.length + "\n");
          if (er) { return cb.call(that, er); }
          if (mt == 2) {
            // might be floats
...
function ParseStream() {
  ...
  this.pipeIn = function(existingStream) {
    existingStream.pipe(bs);
  };

  process.nextTick(getNext);
}

util.inherits(ParseStream, EventEmitter);
