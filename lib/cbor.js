...
function _unpackSmallNumber(ai, buf, cb) {
  if (ai <= 23) {
    return cb(null, new Unallocated(ai));
  }
  switch(ai) {
    ...
    case 28: return cb(null, new Unallocated(buf[0]));
...
function _unpackDate(tag, obj, cb) {
  switch (typeof(obj)) {
    case 'string':
      return cb(null, new Date(obj)); // RFC 3339
    case 'number':
      return cb(null, new Date(obj * 1000));
  }
  cb(new Error('Unsupported date type: ' + typeof(obj)));
}

function _unpackRegex(tag, obj, cb) {
  if (typeof(obj) !== 'string') {
    return cb(new Error("Unsupported RegExp type: " + typeof(obj)));
  }
  cb(null, new RegExp(obj));
}
...
exports.addSemanticTag = function(tag, fun) {
