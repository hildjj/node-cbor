(function() {
  var BREAK, BufferStream, Commented, MT, Simple, assert, async, events, stream, util, utils,
  ...
  module.exports = Commented = (function(_super) {
    ...
    Commented.comment = function(options, cb) {
      var bs, c, opts;
      if (options == null) {
        throw new Error("options is required");
      }
      ...
      c.on('end', function(er, buf) {
        return cb(er, bs.toString('utf8'));
      });
      c.on('error', cb);
      return c.start();
    };
