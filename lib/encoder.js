...
class Encoder extends stream.Transform {
  ...
  _pushDate (gen, obj) {
    return gen._pushTag(TAG.DATE_EPOCH) && gen._pushAny(obj / 1000)
  }
  ...
  _pushRegexp (gen, obj) {
    return gen._pushTag(TAG.REGEXP) && gen._pushAny(obj.source)
  }
  ...
  _pushUrl (gen, obj) {
    return gen._pushTag(TAG.URI) && gen._pushAny(obj.format())
  }
  ...
  _pushMap (gen, obj) {
    ...
    for (let kv of obj) {
      if (!(gen._pushAny(kv[0]) && gen._pushAny(kv[1]))) {
        return false
      }
    }
    return true
  ...
  _pushObject (obj) {
    ...
    for (let j = 0, len2 = keys.length; j < len2; j++) {
      const k = keys[j]
      if (!(this._pushAny(k) && this._pushAny(obj[k]))) {
        return false
      }
    }
    return true
