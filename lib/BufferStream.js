...
util.inherits(EOFError, Error);
function EOFError() {
  Error.call(this, "End of File");
  Error.captureStackTrace(this, EOFError);
}
EOFError.prototype.toString = function() {
  return "End of File";
};
...
BufferStream.prototype._notifyWaiter = function() {
  ...
  var buf = this.read(this.waitingLen);
...
// read(0) gives all
// read(-1) gives first buffer, up to this.length
// else, reads length up to this.length
BufferStream.prototype.read = function(length) {
  ...
  var lenW;
  switch (length) {
    case 0: lenW = this.length; break;
    case -1: lenW = Math.min(this.length, lenZ); break;
    default: lenW = Math.min(this.length, length); break;
  }
